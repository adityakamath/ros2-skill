...
- **`publish-until --euclidean` crashes on composite fields**: passing `--field pose.pose.position` (which resolves to a dict `{x, y, z}`) raised `TypeError: float() argument must be a string or a real number, not 'dict'` surfaced as a confusing "not found or not numeric" error; fixed the euclidean branch of `ConditionMonitor.callback` to auto-expand composite dict fields to their direct numeric children (sorted by key) — `pose.pose.position` becomes `[x, y, z]` scalars automatically; non-numeric children are skipped; fields with no direct numeric values produce a clear error naming the available keys; SKILL.md common-patterns table gains a "shorthand, auto-expanded" row and a composite-expansion note, plus an example
...
- **`services echo` exits after first event, missing request+response pair**: the previous default mode exited immediately after receiving the first event and had a 5 s timeout, so the server-response event was always missed and the service call had to race against the timeout; unified the two modes — `services echo` now always collects all events within the collection window (`--timeout`, default 30 s, or `--duration`), stops early when `--max-messages` events are received (default: unlimited), and returns them all together; a typical service call produces ≥2 events (client-request + server-response); `--max-messages` default changed from 100 to None; SKILL.md documents a background workflow
- **`services echo` QoS mismatch silently dropping events**: service event publisher uses RELIABLE + VOLATILE + depth=1000 QoS; `TopicSubscriber` defaulted to `qos_profile_system_default` which caused a QoS incompatibility and silently dropped all events; added `_get_service_event_qos()` returning a matching `QoSProfile` and passed it to `TopicSubscriber` in `cmd_services_echo`; `TopicSubscriber` gains optional `qos` parameter to accept a caller-supplied QoS profile
