...
- Use `summary.safety_limits.linear_x` as the hard ceiling for `--max-vel` and `summary.safety_limits.angular_z` for `--max-ang` (see Rule 28). `linear_y` is populated for holonomic robots; `null` for diff-drive. Both values are derived from workspace YAML (nav2, controller, and teleop configs) and URDF safety controllers.
