...
- **Navigation:** `summary.active_controllers` lists controller names spawned by launch files. `summary.controller_plugins` lists the raw plugin type strings. `summary.localization_config` and `summary.nav2_config` carry EKF and Nav2 planner/controller plugin details.
