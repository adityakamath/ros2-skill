...
| **— ROBOT PROFILE —** | | |
| "build profile / scan robot / create profile / generate robot profile / scan workspace / analyse workspace / detect robot type / what kind of robot is this" | Build a static robot profile from the workspace | `profile scan` (add `--allow-live` if graph is up; `--name <name>` to override auto-name; `--robot-type TYPE` to override detection) |
