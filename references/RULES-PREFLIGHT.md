...
| Joint-command topic/type/order (pan-tilt, arm, gripper) | `summary.joint_command_topics[]` — `{controller, topic, msg_type, joints, interface_name}` per `ForwardCommandController`/`JointGroupPositionController`/`JointGroupVelocityController`/`JointGroupEffortController` found in the controller YAML. `topic` is always `/<controller>/commands`, `msg_type` is always `std_msgs/msg/Float64MultiArray` — this is fixed ros2_control convention derived from static config, not runtime state, and needs zero live discovery. | `topics list` / `topics find` / `topics type` to discover the topic |
| IMU topic | `summary.imu_topics[]` — `{controller, topic, msg_type}` per `imu_sensor_broadcaster/IMUSensorBroadcaster` found in the controller YAML. `topic` is always `/<controller>/imu`, `msg_type` is always `sensor_msgs/msg/Imu` — same static-convention reasoning as joint-command topics. | `topics find sensor_msgs/msg/Imu` |
| GPS topic | `summary.gps_topics[]` — `{controller, topic, msg_type}` per `gps_sensor_broadcaster/GPSSensorBroadcaster` found in the controller YAML. `topic` is always `/<controller>/gps/fix` (confirmed via the compiled plugin binary — note this is two path segments, unlike IMU's single-segment convention), `msg_type` is always `sensor_msgs/msg/NavSatFix`. | `topics find sensor_msgs/msg/NavSatFix` |
| LiDAR scan topic | `summary.lidar_scan_topic` — normalized from the driver's static `laser_scan_topic_name` YAML config | `topics find sensor_msgs/msg/LaserScan` |
| Diagnostics / joint-states topics | Always `/diagnostics` and `/joint_states` respectively — fixed ROS-wide conventions, not robot-specific, no profile field needed | `topics find diagnostic_msgs/msg/DiagnosticArray` / `topics find sensor_msgs/msg/JointState` |

**`interface proto <type>`** is still required once per session for `Twist`/`TwistStamped` velocity commands — the profile provides the type string but not the field layout. Run it once after reading the type from the profile. **Not needed for joint commands** — `std_msgs/msg/Float64MultiArray`'s shape (`{"data": [...]}`) never varies, so `summary.joint_command_topics[].msg_type` is sufficient on its own.
...
Prefer `preflight motion --controller <NAME> --odom-topic <ODOM_TOPIC>` over running these two as separate calls — one node spin instead of two.

**Before a joint/pan-tilt/arm position command (even when a profile is loaded):**
1. `control list-controllers` — confirm the controller listed in `summary.joint_command_topics[].controller` is currently **active**
2. Subscribe to `/joint_states` for 1 message — read current positions for the joints being commanded

Prefer `preflight joint-command --controller <NAME> --joints <j1,j2,...>` over running these two as separate calls — same one-node-spin savings as `preflight motion`.
...
| Capture a camera image (plain "take a picture" / "grab an image" / "send me a photo" request) | `topics find sensor_msgs/msg/CompressedImage` and `topics find sensor_msgs/msg/Image` to discover available camera topics; use the result in `topics capture-image --topic <CAMERA_TOPIC>`. **No calibration check here** — that already ran once when the camera's launch session came up (Rule 0.6). Re-running it on every capture is a Rule 14 violation (live discovery for data already established this session). |
| Use a camera image or depth image for spatial computation (object detection, depth estimation, object localisation, point cloud processing — i.e. downstream code will use `K`, `frame_id`, or a TF lookup, not just view/send the image) | Before using the camera data this way: 1. Find the paired `camera_info` topic: `topics find sensor_msgs/msg/CameraInfo` — the result will share a namespace with the image topic (e.g., `/camera/camera_info` pairs with `/camera/image_raw`). 2. Subscribe to confirm calibration is present: `topics subscribe <CAMERA_INFO_TOPIC> --max-messages 1 --timeout 2` — verify `K` (intrinsic matrix) is non-zero. 3. Read the `header.frame_id` from the `camera_info` message; confirm it is present in `tf list`. **A camera with no `camera_info` publisher is uncalibrated. A camera whose `frame_id` is absent from TF will produce wrong spatial results.** Both conditions must be satisfied before a spatial-computation task — but this is a session-level check (Rule 0.6), not a per-request one: once confirmed after the camera's launch session came up, do not re-check it for every subsequent spatial task in the same session unless the camera was relaunched. |
...
- ❌ `topics list`/`topics find`/`topics type` to discover a pan-tilt/arm/gripper command topic — it's always `/<controller>/commands` with `std_msgs/msg/Float64MultiArray`, already in `summary.joint_command_topics[]`
...
**Camera calibration check — once per camera launch, not per capture.** If the `launch new` (or `run new`) call just brought up a camera driver (the package/launch file name or profile's `camera_configs` indicates a camera pipeline, e.g. `depthai_ros_driver`, `oakd.launch.py`), verify calibration **once** as part of confirming the bringup succeeded:
1. `topics find sensor_msgs/msg/CameraInfo` — confirm a camera_info topic came up alongside the image topic.
2. `topics subscribe <CAMERA_INFO_TOPIC> --max-messages 1 --timeout 2` — confirm `K` is non-zero.

This is the only point at which calibration is checked. Once confirmed for a given camera launch session, do not re-check it again for that session — not on a later "capture image" request, and not on a later spatial-computation task — unless that camera's launch session is killed and relaunched (a fresh check applies to the new session), or the driver crashes and is restarted (as with the `oak_container` `double free or corruption` case — restart the camera launch, then re-verify once).
