...
- **Velocity ceiling:** use `summary.safety_limits.linear_x` as the `--max-vel` ceiling and `summary.safety_limits.angular_z` as the `--max-ang` ceiling for all motion commands this session. For holonomic robots, also apply `summary.safety_limits.linear_y` as the strafe ceiling. These are static values derived from workspace YAML (nav2, controller, and teleop configs) and URDF safety controllers at scan time. Still run the full four-source live sweep (Rule 0 velocity limit section) before any motion command, but treat the profile limits as a hard upper bound that must not be exceeded regardless of what the live sweep finds.
